...
void aimath_f32_default_matMul_andAddTo(const aitensor_t *a, const aitensor_t *b, aitensor_t *result){
	uint16_t i, j, k;
	float sum;
...
void aimath_f32_sparse_matMul(const aitensor_t *a, const aitensor_t *b, aitensor_t *result, const uint16_t* ind, const uint16_t ind_len)
{
	uint16_t i, j, k;
	float sum;
	...
	for(i = 0; i < a->shape[0]; i++)
	{
		for(j = 0; j < b->shape[1]; j++)
		{
			...
			for(k = 0; k < ind_len; k++)
			{
				sum += a_data[i*a->shape[1] + ind[k]] * b_data[ind[k]*b->shape[1] + j];
			}
			result_data[i*b->shape[1] + j] = sum;
		}
	}
...
void aimath_f32_sparseRowsA_matMul_andAddTo(const aitensor_t *a, const aitensor_t *b, aitensor_t *result, const uint16_t* ind, const uint16_t ind_len)
{
	uint16_t i, j, k;
	float sum;
	...
	for(i = 0; i < ind_len; i++)
	{
		for(j = 0; j < b->shape[1]; j++)
		{
			...
			for(k = 0; k < a->shape[1]; k++)
			{
				sum += a_data[ind[i]*a->shape[1] + k] * b_data[k*b->shape[1] + j];
			}
			result_data[ind[i]*b->shape[1] + j] += sum;
		}
	}
	return;
}

void aimath_f32_sparseColumnsB_matMul_andAddTo(const aitensor_t *a, const aitensor_t *b, aitensor_t *result, const uint16_t* ind, const uint16_t ind_len)
{
	uint16_t i, j, k;
	float sum;
	...
	for(i = 0; i < a->shape[0]; i++)
	{
		for(j = 0; j < ind_len; j++)
		{
			...
			for(k = 0; k < a->shape[1]; k++)
			{
				sum += a_data[i*a->shape[1] + k] * b_data[k*b->shape[1] + ind[j]];
			}
			result_data[i*b->shape[1] + ind[j]] += sum;
		}
	}
	return;
}
...
void getAbsTopKIndices(uint16_t* topK, const uint16_t k, const aitensor_t* tensor, uint16_t row)
{
	if (k > tensor->shape[1])
	{
		LOG_E("K bigger than thre are elements in row\n");
		return;
	}
	if (row >= tensor->shape[0])
	{
		LOG_E("Row is non existing in specified tensor\n");
		return;
	}
	float* data = (float*)tensor->data;
	uint16_t i,j,minIndex;
	int16_t rowOffset = row*tensor->shape[1];
	float minValue = FLT_MAX;
	// initially store values 0 to k-1 in array and memorize lowest entry
	for (i = 0; i < k; i++)
	{
		topK[i] = i;
		if (fabs(data[rowOffset + i]) < minValue)
		{
			minIndex = i;
			minValue = fabs(data[rowOffset + i]);
		}
	}
	// check all remaining n-k elements in row
	for (i = k; i < tensor->shape[1]; i++)
	{
		float temp = fabs(data[rowOffset + i]);
		if (temp > minValue) {
			topK[minIndex] = i;
			// recheck minValue
			minValue = FLT_MAX;
			for (j = 0; j < k; j++)
			{
				if (fabs(data[rowOffset + topK[j]]) < minValue)
				{
					minIndex = j;
					//printf("%i\n",topK[j]);
					minValue = fabs(data[rowOffset + topK[j]]);
				}
			}
		}
	}
...
void getAbsTopKIndices_minHeap(uint16_t* topK, const uint16_t k, const aitensor_t* tensor, uint16_t row)
{
	...
	int i;
	int16_t rowOffset = row*tensor->shape[1];
	// initially store values 0 to k-1 in array and make it a min heap
	for (i = 0; i < k; i++)
	{
		topK[i] = i;
	}
	// catch k=n to not heapify at all and instead return directly!
	if (k == tensor->shape[1])
        return;
    for (i = k >> 1; i >= 0; i--) {
        heapify(topK, data, rowOffset, i, k);
    }
	// check all remaining n-k elements in row
	for (i = k; i < tensor->shape[1]; i++)
	{
		if (fabs(data[rowOffset + i]) > fabs(data[topK[0]])) {
			topK[0] = i;
			heapify(topK, data, rowOffset, 0, k);
		}
	}
}

// not in header to make it "private"
void heapify(uint16_t* topK, const float* data, const uint16_t rowOffset, uint16_t idx, const uint16_t k)
{
    while (1)
    {
        uint16_t leftChild = (idx << 1) + 1, rightChild = leftChild + 1;
        if (leftChild >= k)
            return;
        ...
        if (rightChild >= k)
            ...
            float leftChildValue  = fabs(data[rowOffset + topK[leftChild]]);
            float rightChildValue = fabs(data[rowOffset + topK[rightChild]]);
            nxt = (leftChildValue > rightChildValue) ? rightChild : leftChild;
        }

        float idxValue = fabs(data[rowOffset + topK[idx]]);
        float nxtValue = fabs(data[rowOffset + topK[nxt]]);
        if (idxValue > nxtValue)
        {
            // swap nodes if absolute entry value is higher than absolute entry value in child notd
            uint16_t temp = topK[idx];
            topK[idx] = topK[nxt];
            topK[nxt] = temp;
            idx = nxt;
        }
