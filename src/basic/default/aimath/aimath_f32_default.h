...
/** @brief Performs a matrix multiplication of \link aimath_f32.h F32 \endlink matrices a and b and add result to matrix c (must be zero)
  *
  * @param *a       F32 matrix a (2D tensor of shape [N x K])
  * @param *b       F32 matrix b (2D tensor of shape [K x M])
  * @param *result  Resulting F32 matrix of the multiplication (2D tensor of shape [N x M])
...
/** @brief Performs a sparse matrix multiplication of tensors a and b
  *
  * Matrix multiplication with colums of tensor a and/or rows of tensor b being sparse.
  * Result tensor is fully occupied.
  *
  * (* | | *) (* *)   (* *)
  * (* | | *) (- -) _ (* *)
  * (* | | *) (- -) - (* *)
  ...
  * @param *b       F32 matrix b (2D tensor of shape [K x M])
  * @param *result  Resulting F32 matrix of the multiplication (2D tensor of shape [N x M])
  * @param *ind     K indices being part of the multiplication
  * @param ind_len  number of active indices
*/
...
/** @brief Performs a sparse matrix multiplication of matrices a and b
  *
  * Matrix multiplication with rows of tensor a being sparse
  * Result tensor has not defined entries but because of addTo, only valid entries get added to result tensor,
  * effectively masking the rest with 0
  *
  * (* * * *) (* *)   (* *)
  * (- - - -) (* *) _ (0 0)
  * (- - - -) (* *) - (0 0)
...
/** @brief Performs a sparse matrix multiplication of matrices a and b
  *
  * Matrix multiplication with columns of tensor b being sparse
  * Result tensor has not defined entries but because of addTo, only valid entries get added to result tensor,
  * effectively masking the rest with 0
  *
  * (* * * *) (* |)   (* 0)
...
/** @brief Get indices of top K elements from row of a tensor
 *
 * Time Complexity is O((n-k)*k); slightly worse compared to min heap-method O(n*log(k))
 * but returns array of indices with top K elements, needed for sparse_mat_mul
 *
 * @param *topK     array to store top K values in
 * @param *k        number of top elements to select; size of topK
 * @param *tensor	Tensor to select top K elements from
 * @param *row	    Row to select K elements from
 */
void getAbsTopKIndices(uint16_t* topK, const uint16_t k, const aitensor_t* tensor, uint16_t row);

/** @brief Get indices of top K elements from row of a tensor
 *
 * Time Complexity is O(n*log(k)); slightly worse compared to min heap-method O(n*log(k))
 * but returns array of indices with top K elements, needed for sparse_mat_mul
 *
 * @param *topK     array to store top K values in
 * @param *k        number of top elements to select; size of topK
