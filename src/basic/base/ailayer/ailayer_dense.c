...
{
    layer->maxError = 0;
    layer->requires_grad = 255;          /**< 0: Freezed, 1:Calculate gradient. Each bit represents one trainable param index. All layers active by default */
	...
	layer->base.backward_meProp = ailayer_dense_backward_meProp;
...
void ailayer_dense_backward(ailayer_t *self)
{
	...
	if (layer->requires_grad & 1) {
        //printf("doing layer %i weights\n",layer->base.index);
        aimath_transpose_vector(x_in);
        aimath_f32_default_matMul_andAddTo(x_in, delta_out, d_weights);
        aimath_transpose_vector(x_in);
	}
	// d_bias += delta_out
	if (layer->requires_grad & (1 << 1)) {
        //printf("doing layer %i biases\n",layer->base.index);
	...
	// d_in = d_out * w^T <=> d_in^T = (d_out * w^T)^T = w * d_out^T
	if (layer->base.index > 1) {    //only do this calculation if NOT first dense layer!
...
void ailayer_dense_backward_meProp(ailayer_t *self, float maxBpr, float minBpr, float wearoff_base, int dense_counter)
{
	aitensor_t *delta_in = &(self->deltas);
	aitensor_t *delta_out = &(self->output_layer->deltas);
	aitensor_t *x_in = &(self->input_layer->result);
	...
	// get index of topK elements from delta_out
	float absError = 0.0f;
	for (uint32_t i=0; i<delta_out->shape[1]; i++)
    {
        absError += fabs(((float*)delta_out->data)[i]);
    }
    if (absError > layer->maxError)
        layer->maxError = absError;
    float bpr = (minBpr + absError*(maxBpr-minBpr)/(layer->maxError))*powf(wearoff_base, dense_counter);


    layer->ratio += bpr;  //for plotting average bpr over epoch

	uint16_t k = delta_out->shape[1]*bpr;
	k = (k == 0) ? 1 : k;                                    // at least one neuron
    k = (k > delta_out->shape[1]) ? delta_out->shape[1] : k; // max number of neurons itself
	uint16_t topK[k];
	getAbsTopKIndices_minHeap(topK, k, delta_out, 0);

	// d_weights += x_in^T * delta_out
	if (layer->requires_grad & 1) {
        //printf("doing layer %i weights\n",layer->base.index);
        aimath_transpose_vector(x_in);
        aimath_f32_sparseColumnsB_matMul_andAddTo(x_in, delta_out, d_weights, topK, k);
        aimath_transpose_vector(x_in);
	}
	// d_bias += delta_out
	if (layer->requires_grad & (1 << 1)) {
	...
	if (layer->base.index > 1) {    //only do this calculation if NOT first dense layer!
        aimath_transpose_vector(delta_out);
        aimath_transpose_vector(delta_in);
        aimath_f32_sparse_matMul(weights, delta_out, delta_in, topK, k);
        aimath_transpose_vector(delta_in);
        aimath_transpose_vector(delta_out);
	}
