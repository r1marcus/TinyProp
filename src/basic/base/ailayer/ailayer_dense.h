...
struct ailayer_dense {
	...
	float maxError;
	float ratio;        /**< Providing metric to monitor back propagation ratios */
	uint8_t requires_grad;          /**< 0: Freezed, 1:Calculate gradient. Each bit represents one trainable param index. All layers active by default */
...
/** @brief Calculate the backward pass for the given Dense layer with meProp
 *
 * *Implementation of ailayer.backward_meProp.*
 *
 ...
 * @param dense_counter  iterator counting number of dense_layer starting at the end of the model
 */
