...
uint8_t aialgo_compile_model(aimodel_t *model)
{
	...
	{
	    layer_ptr->index = layer_counter;
		layer_counter++;
		model->trainable_params_count += layer_ptr->trainable_params_count;

		layer_ptr = layer_ptr->output_layer;
	}
	model->layer_count = layer_counter+1;
...
void predict_model_binary_acc(float *model_acc, int num_data, const float threshold, float *data_label, float *model_pred){
	uint8_t argmax_target_acc;
    uint8_t argmax_pred_acc;
    *model_acc = 0;
    ...
    for(int i=0;i<num_data;i++) {
        ...
        argmax_target_acc = *(data_label+i);
        argmax_pred_acc  = (*(model_pred+i) <= threshold) ? 0 : 1;

        if(argmax_target_acc == argmax_pred_acc){
            (*model_acc)++;
        }
    }
    *model_acc = (*model_acc/num_data)*100;
