...
struct ailayer {
	...
	*/
	...
	uint16_t index;    /**< Make position of lasyer in model known to layer to make optimizations in backwards pass! */
	...
	*/
	...
	/** @brief Calculate the backward pass doing meProp and write the result to the deltas tensor.
	*
	* @param self           The layer
	...
	*/
	void (*backward_meProp)(ailayer_t *self, float maxBpr, float minBpr, float dense, int dense_counter);
