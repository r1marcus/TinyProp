...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < 10; i++) {
        for(k = 0; k < 2; k++) {
            ...
            for(j = 0; j < 10; j++) {
                if(k==0){
                    printf("%f",MNIST_training_data_label[i][j]);
                    printf("\t");
                }
                else {
    ...
    predict_model_acc(&training_acc, NUM_TRAIN_DATA, OUTPUTS, &MNIST_training_data_label, &output_data);
    printf("Model accuracy before training: %.3f%%\n\n", training_acc);
