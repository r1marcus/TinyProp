...
#define BACKWARDS_PASS_REPEATS 1000
//#define MONITOR_BP
...
void create_history_data(char *filename, float data[][EPOCHS+1][8], unsigned int *seeds, int trials){
    FILE *fp;
    int i,j, n=8, m=EPOCHS+1;

    filename=strcat(filename,".csv");
    fp=fopen(filename,"w+");

    fprintf(fp,"Epoch,Training Loss,Training Accuracy,Time [s],BPR Layer1,BPR Layer2,BPR Layer3,Test Loss,Test Accuracy,Seed");
...
int main(int argc, char *argv[])
{
    ...
    //dense_layer_2.requires_grad = 0b11111110;
    ...
    // documentation
    float history[trials][EPOCHS+1][8];  //before training + epochs
    ...
    for (int t=0; t<trials; t++) {
        ...
        for(uint32_t i = 0; i < EPOCHS; i++)
        {
            ...
            if (i > 0) {
                ...
                history[t][i][6] = loss_test;
                history[t][i][7] = test_acc;
        ...
        predict_model_acc(&test_acc, NUM_TEST_DATA, OUTPUTS, &IRIS_test_data_label, &output_test_data);
        aialgo_calc_loss_model_f32(&model, &input_tensor_test_data, &target_tensor_test_data, &loss_test);
        history[t][EPOCHS][6] = loss_test/(OUTPUTS * NUM_TEST_DATA);
        history[t][EPOCHS][7] = test_acc;
